...
$strsql = "SELECT item.name, pawn_ticket.loan_amount,
            pawn_ticket.pawn_ticket_id, pawn_ticket.customer_id, pawn_ticket.interest, pawn_ticket.date_of_payment,
            pawn_ticket.redeem_date, item.status FROM item INNER JOIN pawn_ticket ON item.item_id = pawn_ticket.item_id
            WHERE pawn_ticket.customer_id = '$customer_id'"; 
$parameters = array(); //empty initially

//filter added to query
if(isset($_POST['filter'])){
   $filter = $_POST['filter'];
   $strsql .= " AND (item.name LIKE ? OR pawn_ticket.loan_amount LIKE ?
      ...
      <div class="row justify-content-center">
        <div class="col-md-8">
          <div class="bg-slate border p-5 rounded-5" data-aos="fade-up" style="background: linear-gradient(175deg, rgb(104 137 200 / 16%) 0%, rgb(227 93 141 / 12%) 100%)">
            <h1 class="my-3 text-center">Pawn Status of items</h1>
            <form class="filterForm" method="POST" action="cus-pawnticketlist.php?customer_id=<?php echo $customer_id?>">
    ...
    <div>
      ...
      <?php
        if($prepared->rowCount() > 0){
          ...
          echo '<tr>';
          echo '<tr>';
          echo '<th scope="col">Name</a></th>';
          echo '<th scope="col">Pawn Ticket ID</a></th>';
          echo '<th scope="col">Loan Amount</a></th>';
          echo '<th scope="col">Interest(%)</a></th>';
          echo '<th scope="col">Date of Payment</a></th>';
          echo '<th scope="col">Redeem Date</a></th>';
          echo '<th scope="col">Amount Payable (As of today)</a></th>';
          echo '</tr>';
          echo '<thead>';
          echo '<tbody>';
          ...
          while($row= $prepared->fetch()){
            ...
            $date1 = new DateTime($row['date_of_payment']);
            $date2 = new DateTime(date('m/d/Y h:i:s a'));
            $interval = $date1->diff($date2);
            $days = $interval->days;
            $amount_payable = ($days/30)*$row['interest']*$row['loan_amount'] + $row['loan_amount'];
