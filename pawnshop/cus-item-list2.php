...
$strsql = "SELECT item.name, item.item_id, item.brand, item.type, item.model, 
            invoice.selling_price, item.status FROM item INNER JOIN invoice ON item.item_id = invoice.item_id 
            WHERE item.status = 'for sale' AND invoice.action = 'for sale'";
$parameters = array(); //empty initially

//filter added to query
if(isset($_POST['filter'])){
  $filter = $_POST['filter'];
  $strsql .= "AND (item.name LIKE ? OR item.item_id LIKE ? OR item.brand LIKE ? OR item.type LIKE ? OR
            item.model LIKE ? OR item.status LIKE ?)";
  $parameters = ["%{$filter}%","%{$filter}%", "%{$filter}%","%{$filter}%","%{$filter}%","%{$filter}%"];
        ...
        <div class="container-fluid"> <a class="navbar-toggler ms-sm-auto mx-3 me-md-0 p-2 border-0 text-white"
          ...
          <div class="navbar-collapse collapse" id="navbarCategoryCollapse">
            <ul class="navbar-nav navbar-nav-scroll nav-pills-primary-soft text-center p-0">
              ...
              <li class="nav-item active"> <a class="nav-link" href="cus-item-list2.php">Item List</a></li>
      ...
      <div class="row justify-content-center">
        <div class="col-md-8">
          <div class="bg-slate border p-5 rounded-5" data-aos="fade-up" style="background: linear-gradient(175deg, rgb(104 137 200 / 16%) 0%, rgb(227 93 141 / 12%) 100%)">
            ...
            <form class="filterForm" method="POST" action="cus-item-list2.php">
    ...
    <div>
      ...
      <?php
        if($prepared->rowCount() > 0){
          echo '<thead>';
          echo '<tr>';
          echo '<th scope="col"><a href="cus-item-list2.php?sort=name">Name</a></th>';
          echo '<th scope="col"><a href="cus-item-list2.php?sort=item_id">Item ID</a></th>';
          echo '<th scope="col"><a href="cus-item-list2.php?sort=brand">Brand</a></th>';
          echo '<th scope="col"><a href="cus-item-list2.php?sort=type">Type</a></th>';
          echo '<th scope="col"><a href="cus-item-list2.php?sort=model">Model</a></th>';
          echo '<th scope="col"><a href="cus-item-list2.php?sort=status">Status</a></th>';
          echo '<th scope="col"><a href="cus-item-list2.php?sort=selling_price">Price</a></th>';
          echo '</tr>';
          echo '<thead>';
          echo '<tbody>';
        ...
        <div class="row mx-auto">
          <div class="col-md-4 mx-auto text-center">
            ...
            <ul class="nav justify-content-between mt-5 mt-md-0">
              <li class="nav-item"><a href="index.php" class="nav-link p-2">Home</a></li>
